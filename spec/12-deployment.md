...
## Purpose
Define deployment options and CI/CD pipelines for the ERKS MVP, with a primary focus on running on local networks (e.g., Raspberry Pi over LAN) or localhost.
...
- **Configuration**: Application configuration must be loaded via a YAML file corresponding to the schema in `erks/config.py`.
...
## Configuration
The deployment relies on a YAML configuration file to define system parameters, which maps directly to the `Config` dataclass in `erks/config.py`. Users must be able to provide a `config.yaml` file to set properties for:
...
- **Run**: Start the server using Uvicorn, ensuring it binds to all interfaces to allow LAN access.
  - Example: `uvicorn erks.main:app --host 0.0.0.0 --port 8000`
- **Config**: Pass the YAML config file path to the application (e.g., via an environment variable like `ERKS_CONFIG_PATH` or a CLI argument).
...
- **Run**: Mount the YAML config file and necessary storage directories as volumes.
  - Example: `docker run -p 8000:8000 -v ./config.yaml:/app/config.yaml -e ERKS_CONFIG_PATH=/app/config.yaml my-erks-app`
...
## Success Criteria
- ERKS MVP successfully starts and is accessible over LAN from a Raspberry Pi or local machine.
