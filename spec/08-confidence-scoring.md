...
## Purpose
Define a standalone component to compute a hybrid confidence score for retrieved chunks against a user's query. The score combines semantic vector similarity with lexical keyword overlap to provide a calibrated relevance metric.
...
2. **Reuse Vector Scores:** Instead of recomputing embeddings, the scorer accepts the `similarity_score` (which represents cosine similarity, given the L2-normalization in the FAISS index) directly from the `VectorStore`'s search results.
3. **Keyword Overlap Logic:** 
   - Both the query and the chunk text are preprocessed: lowercased, stripped of punctuation, and filtered of common stop words (e.g., "the", "and", "is").
   - The overlap is calculated as the proportion of processed query keywords that appear in the processed chunk text.
4. **Weighted Combination:** The final confidence score is a weighted sum: **70% Cosine Similarity** and **30% Keyword Overlap**.
5. **Per Chunk Scoring:** The score is calculated individually per chunk evaluated.
...
- A predefined list of common English stop words is filtered out of both the query tokens and the chunk tokens.

### 2. Keyword Overlap Calculation
- Calculate the set of unique `query_keywords`.
- Calculate the set of unique `chunk_keywords`.
...
class ConfidenceScorer:
    ...
    def score(self, query: str, chunk_text: str, vector_score: float) -> float:
        ...
        chunk_keywords = self._preprocess(chunk_text)
...
- **Determinism:** The same query, chunk text, and vector score always yield the exact same confidence score.
